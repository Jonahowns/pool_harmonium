...
from pool.io import fasta_read_basic, gunter_read, csv_read, write_fasta
...
def enrichment_average(df, label_names, min_diff=1, max_diff=None, diff_weights=None, label_weights=None):
    ...
    if label_weights is None:
        label_weights = [1. for x in range(len(label_names))]
...
def data_properties(seqs, label=None, outfile=sys.stdout, calculate_copy_number=True):
    ...
    if label is not None:
        data.update({"label": [label for x in useqs]})
...
def copynumber_topology(dataframe, rounds, threads_per_task=1):
    dfs = []
    for r in rounds:
